...
<head>
  ...
  <title>jenis | CRUD Data Makanan</title>
  <!-- Required meta tags -->
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <!-- bootstrap template -->
    ...
    <div class="card shadow mt-3">
      <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
          <label class="h6">Data Kategori</label>
          <a href="./kategori-form.php" class="btn btn-sm btn-info">Tambah</a>
        </div>
      </div>
      <div class="card-body">
        <table class="table table-responsive-md table-striped table-hover">
          ...
          <tbody>
            <?php
            $query = "SELECT pt.*, (SELECT count(*) FROM makanan as bu WHERE pt.id = bu.kategori_id ) as makanan FROM kategori as pt";
            ...
            while ($row = mysqli_fetch_assoc($result)) {
              ...
              $btn_edit =  '<a href="./kategori-form.php?edit=' . $row->id . '" class="btn btn-sm btn-primary">Edit</a>';
              $btn_delete =  '<a href="./kategori.php?delete=' . $row->id . '" onclick="return confirm(\'Apakah anda yakin?\')" class="btn btn-sm btn-danger">Hapus</a>';
              echo "<tr>
              <th>$counter</th>
              <td>{$row->nama}</td>
              <td>{$row->makanan}</td>
